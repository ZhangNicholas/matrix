...
private:
	...
	{
	public:
		Cell(PseudoMatrix* matrix_, int i, int j) : _matrix(matrix_), i(i), j(j) {}
	...
	{
	public:
		Proxy(PseudoMatrix* matrix_, int i) : _matrix(matrix_), i(i) {}
	...
	public:
		...
		const std::tuple<int, int, T> operator*() const
		{
